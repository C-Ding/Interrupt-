...
SOMEDELAY  EQU 500000					; 0.1 Second
;SOMEDELAY  EQU 1
Start
	...
	MOV R8, #0				; turn off LEDS
	BL display_8_bits
	MOV R11, #14153				; seed for RNG
	BL Generate_random		; generate and store a random number 50-253 in R8

countdown	
	MOV R8, R6
	MOV R5, R6
	BL display_8_bits		; display the number
	BL display_right_7seg
	BL display_left_7seg

	MOV R0, #10				; get ready to delay 10 * 0.1s = 1 s
	BL MiliDelay			; delay 1 s
	SUBS R6, #10			; subtract 1s from R8
	
	BPL countdown			; if not negative, repeat
	BMI display_zero		; else store 0 in R8 and display it
...
Generate_random 	STMFD		R13!,{R0, R1, R14}

	BL RandomNum
	
	MOV R0, R11				; move the random 16 bit number to R0
	;MOV R0, #0xFF
	AND R0, #0x7			; and it with 0x7 so that only the lowest 3 bits are used
	
	MOV R1, #29			; we have a range of 0-7(3 bits) so we can multiply by 29
	MUL R0, R1				; now R0 is in the of range 0 - 203
	
	MOV R1, #50
	ADD R0, R1				; add 50 to R0 - now in the range 50 - 253(5.0 sec - 24.9920 sec)
							; which is within the absolute error of 0.5 sec		
	MOV R6, R0				; Simply moving the random number into R6
	
					LDMFD R13!,{R0, R1, R15}
	
MiliDelay 			STMFD		R13!,{R2, R14}
	LDR R2, =SOMEDELAY			; load 0.1s into R2
...
display_8_bits 		STMFD		R13!,{R2, R3, R14}		; display the 8-bit value in register R8
	LDR R3, =GPIO_PORTB + (PORT_B_MASK << 2)		; generate the base address for port B
	STR R8, [R3, #GPIO_DATA_OFFSET]
   ...
   ; LDR R1, [R3, #GPIO_DATA_OFFSET]		; GPIO_DATA_OFFSET is the address offset to get to the DATA part of the port
	STR R8, [R3, #GPIO_DATA_OFFSET]		; write the data back out
...
GPIOPortF_Handler	; an interrupt due to any pin on Port F calls this interrupt handler
	...
	STMFD		R13!,{R14}		; I'm paranoid - save R1 and R3 since I'm using them
	...
	SUB R11, R11, #1				; subtract R11 by 1 in order to get a 'new' seed... 
	BL Generate_random
...
;   There is no harm in clearing extra possibly interrupt sources
    ...
    MOV R0, #0x10             		; clear the interrupt by having a one match the correct bit where the switch is
    STR R0, [R1]	; acknowledge the interrupt
	
	
	LDMFD		R13!,{R15}		; restore the saved registers and return
...
; The switches are bits 0 and 4 on Port F and a 1 must be written to the bit / switch used.
    MOV R0, #0x01            ; 0 means mask or block interrupts
...
display_left_7seg		STMFD		R13!,{R0, R1, R2, R3, R4, R5, R14}

; First take the 4 MSB and display that to HEX #2
; Convert the binary number to the bit pattern required by the 7-segment display

	LDR R3, =Seven_Seg_Table	; get the base address of the 7-segment pattern table
	AND	R5, #0xf0		; mask the number to display to the range 0 to 0xf
	LSR R5, #4
	LSL R5, #1			; multiply by two, to get an address offset in bytes, as the lookup table holds an array of 16-bit data
	LDRH R0, [R3, R5]		; read the 7-segment pattern - table base address + offset in R5
	; DP is Bit 0, then A, B, C, D, E, F, G (bit 7)
...
; use right shift - the LSB that falls off goes into the Carry flag and is used to conditionally set
	...
	MOV	R1, #0		; R1 will hold the data written to port F
	MOV R2, #0		; R2 will hold the data written to port D
	MOV R3, #0		; R3 will hold the data written to port E
...
; NOTE: a "1" in the 7-seg pattern should write a 0 because the 7-seg display is active low - so the code below inverts
	...
	LSRS R0, #1		; shift next LSB - ??//this is segment "G"//?? and goes to port F bit 2 (F2)
	ORRCC R1, #0x04		; conditionally set F2 if Carry is Equal to 0

	LSRS R0, #1
	ORRCC R1, #0x08		; conditionally set F3 if Carry is Equal to 0
	
	LSRS R0, #1
	ORRCC R2, #0x04		; conditionally set D2 if Carry is Equal to 0
	
	LSRS R0, #1
	ORRCC R2, #0x08		; conditionally set D3 if Carry is Equal to 0
	
	LSRS R0, #1
	ORRCC R2, #0x40		; conditionally set D6 if Carry is Equal to 0
	
	LSRS R0, #1
	ORRCC R2, #0x80		; conditionally set D7 - bit 7 if Carry is Equal to 0
	
	LSRS R0, #1
	ORRCC R3, #0x01		; conditionally set E0 - bit 0 if Carry is Equal to 0
	
; now read-modify-write port F
	LDR R0, =GPIO_PORTF + (PORT_F_MASK << 2)
	LDR R4, [R0, #GPIO_DATA_OFFSET]		; read port F
	AND R4, #0xf3		; clear all bits being overwritten from the data manipulation above
	ORR R4, R1			; ORR in the data manipulation done - R1 which is port F data
	STR R4, [R0, #GPIO_DATA_OFFSET]		; write port F back out
	
; now read-modify-write port D
	LDR R0, =GPIO_PORTD + (PORT_D_MASK << 2)
	LDR R4, [R0, #GPIO_DATA_OFFSET]		; read port D
	AND R4, #0x33		; clear all bits being overwritten from the data manipulation above
	ORR R4, R2			; ORR in the data manipulation done - R2 which is port D data
	STR R4, [R0, #GPIO_DATA_OFFSET]		; write port D back out

; now read-modify-write port E
	LDR R0, =GPIO_PORTE + (PORT_E_MASK << 2)
	LDR R4, [R0, #GPIO_DATA_OFFSET]		; read port E
	AND R4, #0xFE		; clear all bits being overwritten from the data manipulation above
	ORR R4, R3			; ORR in the data manipulation done - R3 which is port E data
	STR R4, [R0, #GPIO_DATA_OFFSET]		; write port E back out
...
;
; INPUT: R11 - before calling this for the FIRST time R11 must be initialized to a non-zero value or else it will stay stuck at 0
; OUTPUT: R11 - random number is the lowest 16 bits of R11
;
RandomNum		STMFD		R13!,{R1, R2, R3, R14}
				...
				LSL			R11, #1
				ORR			R11, R11, R3
				MOV			R3, #0xFFFF		; zero all bits above the pseudorandom 16 bits
				AND			R11, R3
